...
class Grid
{
    ...
    public:
        ...
        double getareaXi(int i,int j) const {            return S_xi_2D[i][j];        }
        double getnxXi  (int i,int j) const { return S_xi_x_2D[i][j] / S_xi_2D[i][j]; }
        double getnyXi  (int i,int j) const { return S_xi_y_2D[i][j] / S_xi_2D[i][j]; }
        
        double getareaEta(int i,int j) const {           return S_eta_2D[i][j];          }
